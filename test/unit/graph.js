...
(function () {
  ...
  test("Graph equals() test", function () {
    ...
    ok(
      !graph1.equals(graph2, { css: "background-color" }),
    ...
    ok(
      graph1.equals(graph2, { css: ["stroke", "background-color"] }),
      "Same edge colors"
    );
    ...
    ok(
      graph1.equals(
        graph2,
        { class: "someClass" },
        "Class that exists in same nodes"
      )
    );
    ok(
      !graph1.equals(
        graph2,
        { class: "someClass2" },
        "Class that is not in the other graph"
      )
    );
    ok(
      graph1.equals(
        ...
        "Array of class names that match"
      )
    );
    ok(
      !graph1.equals(
        graph2,
        { class: ["someClass", "someClass2", "unknownClass"] },
        "Array of class names that don't match"
      )
    );
  });
  ...
  test("Graph clone() test", function () {
    ...
    ok(
      graph.equals(graph.clone(), { css: "background-color" }),
      "Cloning a graph with node highlight"
    );

    e.highlight();

    ok(
      graph.equals(graph.clone(), { css: ["stroke", "background-color"] }),
      "Cloning a graph with edge highlight"
    );
    ...
    ok(
      weightedGraph.equals(weightedGraph.clone()),
      "Cloning a graph with edge weights"
    );
  });
  ...
  test("Graph state() test", function () {
    ...
    ok(
      !graph1.equals(graph2, { css: ["color", "background-color"] }),
      "Different graphs should not be equal"
    );
    ok(
      !graph1.equals(graph2, {
        class: ["testing", "testing2", "jsavhighlight"],
    ...
    ok(
      graph1.equals(graph2, { css: ["color", "background-color"] }),
      "After setting state, graphs should be equal"
    );
    ok(
      graph1.equals(graph2, {
        class: ["testing", "testing2", "jsavhighlight"],
    ...
    ok(
      !graph1.equals(graph3, { css: ["color", "background-color"] }),
      "Different graphs should not be equal"
    );
    ok(
      !graph1.equals(graph3, {
        class: ["testing", "testing2", "jsavhighlight"],
        css: ["color", "background-color"],
      }),
      "Different graphs should not be equal"
