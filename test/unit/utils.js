...
(function () {
  ...
  test("numKey(s)", function () {
    ...
    // default sorting
    r.seedrandom(fixedseed);
    deepEqual(r.numKeys(1, 99, 10, { sorted: true }), [
      8,
      9,
      26,
      31,
  ...
  test("sample", function () {
    ...
    equal(
      r.sample([], 1),
      undefined,
      "Sampling more than there are items should return undefined"
    );
    equal(r.sample([2, 4], 3), undefined);
    equal(
      r.sample([2, 4], -3),
      undefined,
      "Negative values should return undefined"
    );
    deepEqual(
      r.sample([2, 5], 0),
      [],
      "Sampling 0 items should return an empty array"
    );
