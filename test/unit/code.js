...
(function () {
  ...
  test("Test show/hide", function () {
    var av = new JSAV("emptycontainer"),
      var1 = av.variable(7);
    equal(
      var1.element.filter(":visible").size(),
      0,
      "Variable initially invisible"
    );
    var1.show();
    av.step();
    equal(
      var1.element.filter(":visible").size(),
      1,
      "Variable again visible after show"
    );
    var1.show();
    av.step();
    equal(
      var1.element.filter(":visible").size(),
      1,
      "Variable visible after another show"
    );
    var1.hide();
    av.step();
    equal(
      var1.element.filter(":visible").size(),
      0,
      "Variable not visible after hide"
    );
    var1.hide();
    ...
    equal(
      var1.element.filter(":visible").size(),
      0,
      "Variable not visible after another hide"
    );
    av.recorded();
    jQuery.fx.off = true;
    av.end();
  ...
  test("Variable equals", function () {
    ...
    ok(
      !var1.equals(var2),
      "Variables with different values shouldn't be equal"
    );
    var2.value("kissa");
    ...
    var1.highlight();
    ok(
      !var1.equals(var2, { class: "jsavhighlight" }),
      "Variables with different highlights shouldn't be equal"
    );
    ok(
      !var1.equals(var2, { css: "background-color" }),
      "Variables with different highlights shouldn't be equal"
    );
    ok(
      var1.equals(var2),
      "Variables with different highlights should be equal if not comparing styles"
    );

    var2.highlight();
    ok(
      var1.equals(var2, { class: "jsavhighlight" }),
      "Variables with same highlights should be equal"
    );
    ok(
      var1.equals(var2, { css: "background-color" }),
      "Variables with same highlights shouldn't be equal"
    );
    ok(
      var1.equals(var2),
      "Variables with same highlights should be equal even if not comparing styles"
    );
    var1.unhighlight();
    var2.unhighlight();
    ...
    ok(
      !var1.equals(var2, { class: "testing" }),
      "Variables with different classes shouldn't be equal"
    );
    ok(
      var1.equals(var2, { class: "testingInvalid" }),
      "Variables with different classes should be equal if not testing for that class"
    );
    ok(
      var1.equals(var2),
      "Variables with different classes should be equal if not comparing classes"
    );

    var2.addClass("testing");
    ok(
      var1.equals(var2, { class: "testing" }),
      "Variables with same classes should be equal"
    );
    ok(
      var1.equals(var2, { class: "testingInvalid" }),
      "Variables with same classes should be equal even if not testing for that class"
    );
    ok(
      var1.equals(var2),
      "Variables with same classes should be equal even if not comparing classes"
    );

    var1.css({ color: "red" });
    ok(
      !var1.equals(var2, { css: "color" }),
      "Variables with different style shouldn't be equal"
    );
    ok(
      !var1.equals(var2, { css: ["font-size", "color"] }),
      "Variables with different style shouldn't be equal"
    );
    ok(
      var1.equals(var2, { css: "font-size" }),
      "Variables with different styles should be equal if not testing for that property"
    );
    ok(
      var1.equals(var2),
      "Variables with different styles should be equal if not comparing styles"
    );

    var2.css({ color: "red" });
    ok(
      var1.equals(var2, { css: "color" }),
      "Variables with same style should be equal"
    );
    ok(
      var1.equals(var2, { css: ["font-size", "color"] }),
      "Variables with same style should be equal"
    ...
    ok(
      var1.equals(var2, { css: "font-size" }),
      "Variables with same styles should be equal even if not testing for that property"
    );
    ok(
      var1.equals(var2),
      "Variables with same styles should be equal even if not comparing styles"
    );
  });
  ...
  test("Pseudocode setCurrentLine", function () {
    ...
    var testCurrents = function (curr, prev) {
      ...
      equal(
        $curr.size(),
        curr === -1 ? 0 : 1,
        "Testing number of current lines"
      );
      equal(
        $prev.size(),
        prev === -1 ? 0 : 1,
        "Testing number of previous lines"
      );
    };
    av.displayInit();
    testCurrents(-1, -1);
  ...
  test("Pseudocode options", function () {
    ...
    equal(
      pseudo.element.find(".jsavcodeline:eq(2)").html(),
      "line3 &lt;br/&gt; line4"
    );
    equal(
      pseudo.element.find(".jsavcodeline:eq(2)").text(),
      "line3 <br/> line4"
    );
    // html escaping disabled
    equal(
      pseudoNoEscape.element.find(".jsavcodeline:eq(2)").html(),
      "line3 <br> line4"
    );
    equal(
      pseudoNoEscape.element.find(".jsavcodeline:eq(2)").text(),
      "line3  line4"
    );
  });
  ...
  test("Pseudocode tags", function () {
    ...
    function testCSS(index, css, expectedValue) {
      equal(
        pseudo.element.find(".jsavcodeline:eq(" + index + ")").css(css),
        expectedValue
      );
    }
    function testClass(index, testclass) {
      ok(
        pseudo.element
          .find(".jsavcodeline:eq(" + index + ")")
          .hasClass(testclass),
        "Line " + (index + 1) + " should have class " + testclass
      );
    }
  ...
  test("Pointers positioning", function (assert) {
    ...
    assert.close(
      ptrOffset.top + ptr.element.outerHeight(),
      targetOffset.top,
      maxDifference,
    ...
    assert.close(
      ...
      targetOffset.top,
      maxDifference,
      "Pointer top position after changing target"
    );
    assert.close(
      ptrOffset.left,
      targetOffset.left,
      maxDifference,
      "Pointer left position after changing target"
    );
    ...
    assert.close(
      ...
      targetOffset.top,
      maxDifference,
      "Pointer top position after moving target"
    );
    assert.close(
      ptrOffset.left,
      targetOffset.left,
      maxDifference,
      "Pointer left position after moving target"
    );
    ...
    assert.close(
      ...
      targetOffset.top,
      maxDifference,
      "Pointer top position after changing target to another structure"
    );
    assert.close(
      ptrOffset.left,
      targetOffset.left,
      maxDifference,
      "Pointer left position after changing target to another structure"
    );
    ...
    assert.close(
      ...
      targetOffset.top,
      maxDifference,
      "Pointer top position after moving target through layout"
    );
    assert.close(
      ptrOffset.left,
      targetOffset.left,
      maxDifference,
      "Pointer left position after moving target through layout"
    );
  });
  ...
  test("Fixed pointer positioning", function (assert) {
    ...
    av.displayInit();
    assert.close(
      ptrOffset.top + 300 + ptr.element.outerHeight(),
      targetOffset.top,
      maxDifference,
      "Pointer top position"
    );
    ...
    equal(
      ptrOffset.top,
      origPointerOffset.top,
      "Pointer top position after changing target"
    );
    equal(
      ptrOffset.left,
      origPointerOffset.left,
      "Pointer left position after changing target"
    );
    ...
    equal(
      origArrowBounds.top,
      arrowBounds.top,
      "Arrow still starting from the same top position"
    );
    equal(
      origArrowBounds.left,
      arrowBounds.left,
      "Arrow still starting from the same left position"
    );
