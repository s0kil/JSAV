...
(function () {
  ...
  test("Highlighting indices in Array", function () {
    ...
    av.step();
    arr.highlight(function (index) {
      return index > 3;
    }); // highlight with function
    av.step();
  ...
  test("Unhighlighting indices in Array", function () {
    ...
    av.step();
    arr.unhighlight(function (index) {
      return index > 3;
    }); // highlight with function
    av.step();
  ...
  test("Comparing arrays", function () {
    ...
    ok(
      arr2.equals(arr3, { css: "background-color" }),
      "Equals values and background-colors"
    );
    ok(
      arr2.equals(arr3, { css: ["color", "background-color"] }),
      "Equal values, background-colors and colors"
    );
    ok(
      arr1.equals(arr2, { css: "background-color", value: false }),
      "Ignoring values, equal background-colors"
    );
    arr2.highlight(2);
    av.step();
    ok(
      !arr2.equals(arr3, { css: "background-color" }),
      "Unequal background-colors"
    );

    ok(
      !arr2.equals(arr3, { css: ["color", "background-color"] }),
      "Unequal background-colors and colors"
    );

    var testDiv = $(
        '<ol class="' +
          arr1.element[0].className +
          '"><li class="jsavnode jsavindex jsavhighlight"></li><li class="jsavnode jsavindex" ></li></ol>'
      ),
    ...
    ok(
      arr2.equals([unhlBg, unhlBg, hlBg, unhlBg], { css: "background-color" }),
      "Equal background-colors as array."
    );
    ...
    ok(
      arr1.equals(
        arr3,
        { class: "someClazz", value: false },
        "Arrays with matching classes"
      )
    );
    ok(
      !arr1.equals(
        arr2,
        { class: "someClazz2" },
        "Arrays with non-matching class"
      )
    );
    ok(
      arr1.equals(
        arr3,
        { class: "unknownClazz2", value: false },
        "Non-existing class"
      )
    );
    ok(
      arr1.equals(
        arr3,
        { class: ["someClazz", "unknownClazz2"], value: false },
        "Multiple classes"
      )
    );
    ok(
      !arr1.equals(
        arr3,
        { class: ["someClazz", "someClazz2", "unknownClazz2"], value: false },
        "Multiple, differing classes"
      )
    );
  });
  ...
  test("Test show/hide", function () {
    ...
    av.step();
    equal(
      arr.element.filter(":visible").size(),
      0,
      "Array not visible after hide"
    );
    arr.show();
    av.step();
    equal(
      arr.element.filter(":visible").size(),
      1,
      "Array again visible after show"
    );
    arr.show();
    av.step();
    equal(
      arr.element.filter(":visible").size(),
      1,
      "Array visible after another show"
    );
    arr.hide();
    av.step();
    equal(
    ...
    equal(
      arr.element.filter(":visible").size(),
      0,
      "Array not visible after another hide"
    );
    av.recorded();
    jQuery.fx.off = true;
    av.end();
  ...
  test("Test array state", function () {
    ...
    ok(
      !arr1.equals(arr2, { css: ["color", "background-color"] }),
    ...
    ok(
      arr1.equals(arr2, { css: ["color", "background-color"] }),
      "After setting state, arrays should be equal"
    );
    ok(
      arr1.equals(arr2, {
        css: ["color", "background-color"],
    ...
    deepEqual(
      arr1._values,
      arr2._values,
      "Internal values of arrays should be equal after setting state"
    );

    ok(!arr3.equals(arr1), "Different arrays should not be equal");
    ok(
      !arr3.equals(arr1, { css: ["color", "background-color"] }),
      "Different arrays should not be equal"
    );
    ok(
      !arr3.equals(arr1, {
