...
(function ($) {
    ...
    if (typeof newState !== "undefined") {
      ...
      var setNodeState = function (node, newState) {
        ...
        var newChildState = newChildren[pos],
          child = children[pos];
        while (newChildState && child) {
          // set states of "common" children
  ...
  nodeproto.equals = function (otherNode, options) {
    ...
    // compare edge style
    if (this.edgeToParent()) {
      var edgeEquals = this.edgeToParent().equals(
    ...
    if (typeof newState !== "undefined") {
      ...
      var setNodeState = function (node, state) {
        ...
        var setChild = function (childName) {
          ...
          if (!child && !state[childName]) {
            // no such child
            return;
          } else if (child && state[childName]) {
            // both such children
            setNodeState(child, state[childName]);
          } else if (child) {
            // existing child, need to remove
            node[childName](null, { record: false });
          } else {
            // no existing child but new child, need to add node
  ...
  function setchild(self, pos, node, options) {
    ...
    if (typeof node === "undefined") {
      ...
      if (node === null) {
        // node is null, remove child
        if (child && child.value() !== "jsavnull") {
          child.parent(null, opts);
          // child exists
          if (!oChild || oChild.value() === "jsavnull") {
            // ..but no other child
            if (opts.hide) {
              child.hide();
            }
        ...
        // create a new node and set the child
...
(function ($) {
  ...
  function treeLayout(tree, options) {
      ...
      calculateContours = function (node) {
        ...
        if (children.length === 0) {
          ...
          for (var i = 1, l = children.length; i < l; i++) {
            ...
            translateNodes(
              child,
              getXTranslation(firstChild) + trans - getXTranslation(child),
              NODEGAP + nodeHeight
            );
          }
    ...
    if (!opts.boundsOnly) {
      // only change pos if we are not just calculating bounds
      $.each(results, function (key, value) {
    ...
    if (!opts.boundsOnly) {
      // only go through edges if we are not just calculating bounds
      if (!tree._layoutDone) {
      ...
      $.each(results, function (key, value) {
        ...
        if (node._edgetoparent) {
          var start = {
            ...
            end = { left: endnode.width - minX, top: endnode.height - minY };
