...
(function ($) {
  ...
  MessageHandler.prototype.umsg = JSAV.anim(function (msg, options) {
    ...
    if (this.output) {
      if (this.output.hasClass("jsavline") && opts.preserve) {
        ...
        if (el.size() > 0) {
          // existing content in message output
          el.append(newmsg);
        } else {
          // first message
          this.output.html(
        ...
        // e.g. "jsavscroll", which is default
        this.output.append(
          "<div style='color:" + opts.color + ";'>" + msg + "</div>"
        );
        if (this.output[0]) {
    ...
    if (!this.jsav.RECORD) {
      // trigger events only if not recording
  ...
  JSAV.ext.textToSpeech = function (speechText, options) {
    ...
    var replacements =
      options.overrideReplacements || options.replacements || [];
  ...
  var soundSettings = function (jsav) {
    ...
    $elem.click(function () {
      for (var j = 0; j < window.length; j++) {
        $(".sound", window.frames[j].document)
          .not(this)
          .each(function () {
            $(this.offsetParent)
              .parent()
      ...
      if ($(this.offsetParent).attr("voice") !== undefined) {
        ...
        $(this.offsetParent).parent().find(".jsavcontainer").attr("voice") ===
        "true"
      ) {
        $(this.offsetParent)
          .parent()
  ...
  JSAV.init(function (options) {
    ...
    if (supportsSpeech && options.narration && options.narration.enabled) {
      ...
      if (
        $(this.container).parent().find(".new").length === 0 &&
