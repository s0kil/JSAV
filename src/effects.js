...
(function ($) {
  ...
  var parseValueEffectParameters = function () {
    ...
    if (typeof arguments[secondstrPos + 1] === "number") {
      // array index
  ...
  var doValueEffect = function (opts) {
    ...
    if (opts.mode === "swap") {
      ...
      from.value.apply(
        from,
        opts.args1.concat([
          typeof opts.old !== "undefined" ? opts.old : "",
          { record: false },
        ])
      );
    }
    ...
    if (from.constructor === JSAV._types.ds.AVArray) {
      $fromValElem = from.element.find(
        "li:eq(" + opts.args1[0] + ") .jsavvaluelabel"
      );
    } else if (from.element.hasClass("jsavlabel")) {
    ...
    if (to.constructor === JSAV._types.ds.AVArray) {
      $toValElem = to.element.find(
        "li:eq(" + opts.args2[0] + ") .jsavvaluelabel"
      );
    } else if (to.element.hasClass("jsavlabel")) {
      // special treatment for labels
    ...
    if (this._shouldAnimate()) {
      ...
      if (opts.mode === "swap") {
        ...
        if (from.options.layout !== "bar") {
          ...
          var bottom =
            $fromValElem.parent().height() -
            $fromValElem.position().top -
            fromValElemHeight;
  ...
  JSAV.ext.effects = {
    ...
    swap: function ($str1, $str2, options) {
      ...
      if (this._shouldAnimate()) {
        // only animate when playing, not when recording
        if ("Raphael" in window && opts.arrow) {
          // draw arrows only if Raphael is loaded
