...
(function ($) {
  ...
  graphproto.removeEdge = function (fNode, tNode, options) {
    ...
    var fromIndex = this._nodes.indexOf(fromNode),
      ...
      newAdjlist = adjlist
        .slice(0, edgeIndex)
        .concat(adjlist.slice(edgeIndex + 1));
  ...
  var dagreLayout = function (graph, options) {
    ...
    if (!("dagre" in window)) {
      console.error(
        "You are trying to use the layered layout, please load dagre.js " +
  ...
  nodeproto.equals = function (otherNode, options) {
    ...
    for (i = myNeighbors.length; i--; ) {
      ...
      if (
        !this.container.getEdge(this, myNeighbor).equals(
