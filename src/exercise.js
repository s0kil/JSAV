...
(function ($) {
  ...
  var Exercise = function (jsav, options) {
    ...
    if (
      $jsavscore.size() === 1 &&
      $jsavscore.children().size() === 0 &&
      this.options.feedback === "continuous"
    ) {
      $jsavscore.html(
        this.jsav._translate("scoreLabel") +
          ' <span class="jsavcurrentscore"></span> / ' +
          '<span class="jsavmaxscore" ></span>, <span class="jsavamidone">' +
  ...
  exerproto.grade = function (continuousMode) {
    ...
    if ($.isFunction(this.options.grader)) {
      ...
      graders[this.options.grader + (continuousMode ? "-continuous" : "")].call(
        this
      );
    }
    if (!continuousMode) {
  ...
  exerproto.modelanswer = function (returnToStep) {
    ...
    var model = this.options.model,
      ...
      ); // options passed for the model answer window
  ...
  exerproto.undo = function () {
    ...
    if (
      undoGraders.indexOf(this.options.grader) !== -1 &&
  ...
  exerproto.gradeableStep = function () {
    ...
    var that = this;
    if (
      (this.feedback && this.feedback.val() === "continuous") ||
      ...
      var doContinuousGrading = function () {
        ...
        if (grade.student === grade.correct) {
          // all student's steps are correct
          that.jsav.logEvent({
        ...
        if (grade.correct === grade.total) {
          // student continues with the exercise even after done
          return;
        }
