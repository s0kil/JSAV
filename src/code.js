...
(function ($) {
  ...
  var Variable = function (jsav, value, options) {
    ...
    this.element = $(
      '<div class="jsavvariable">' +
        '<span class="jsavvarlabel"></span> <span class="jsavvalue">' +
        '<span class="jsavvaluelabel jsavvarvalue">' +
        value +
        "</span></span></div>"
    );
  ...
  varproto.state = function (newstate) {
    if (newstate) {
      ...
      // no new state, so return the current state
  ...
  var Pointer = function (jsav, name, options) {
    ...
    JSAV.utils._helpers.setRelativePositioning(
      this,
      $.extend({}, this.options, { callback: this._arrowPointerCallback })
    );
  ...
  pointerproto._translateArrowPoints = function (
    startDiffX,
    startDiffY,
    endDiffX,
    endDiffY
  ...
  pointerproto._setTarget = JSAV.anim(function (newTarget, options) {
  ...
  pointerproto._arrowPoints = function (newLeft, newTop, options) {
    ...
    var targetPoint = this._calculateArrowTargetPosition(this._target, opts),
      newPoints = [
        [
          0,
          (newLeft || myBounds.left) + myBounds.width / 2 + 1, //+1 to center the arrow start "ball"
  ...
  pointerproto._calculateArrowTargetPosition = function (target, opts) {
    ...
    return [
      targetBounds.left + (targetBounds.width * arrowHorzAnchor) / 100.0,
  ...
  pointerproto.target = function (newTarget, options) {
    if (typeof newTarget === "undefined") {
      ...
      if (!this.options.fixed) {
        var newPos = JSAV.utils._helpers.setRelativePositioning(
          this,
      ...
      var newTargetArrowPoint = this._calculateArrowTargetPosition(
        newTarget,
        opts
      );
  ...
  var Code = function (jsav, codelines, options) {
    ...
    this.options = $.extend(
      { visible: true, lineNumbers: true, htmlEscape: true, center: true },
      options
    );
  ...
  function getCodeLineElements(self, linenum) {
    if (typeof linenum === "string") {
      if (
        self.options.tags &&
        typeof self.options.tags[linenum] !== "undefined"
      ) {
  ...
  codeproto.setCurrentLine = function (linenum, options) {
    ...
    if (typeof linenum === "string") {
      if (
        this.options.tags &&
        typeof this.options.tags[linenum] !== "undefined"
      ) {
  ...
  codeproto.css = function (linenum, cssprop, options) {
    ...
    if (typeof cssprop === "string") {
      ...
      if ($.isFunction(linenum)) {
        // if linenum is a function, evaluate it right away and get a list of indices
  ...
  codeproto.show = function (linenum, options) {
    if (
      (typeof linenum === "undefined" || typeof linenum === "object") &&
      !$.isArray(linenum) &&
      this.element.filter(":visible").size() === 0
    ) {
  ...
  codeproto.hide = function (linenum, options) {
    if (
      (typeof linenum === "undefined" || typeof linenum === "object") &&
      !$.isArray(linenum) &&
      this.element.filter(":visible").size() === 1
    ) {
      return this._toggleVisible(linenum);
    } else {
