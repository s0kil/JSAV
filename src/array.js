...
(function ($) {
  ...
  var AVArray = function (jsav, element, options) {
    ...
    this.options = $.extend(
      true,
      { autoresize: true, center: true, layout: "array" },
      options
    );
    if (!this.options.template) {
      this.options.template =
        templates[
          this.options.layout + (this.options.indexed ? "-indexed" : "")
        ];
    }
    this._indices = [];
    if ($.isArray(element)) {
  ...
  arrproto.clone = function () {
    ...
    var newArray = new AVArray(
      this.jsav,
      vals,
      $.extend(true, {}, this.options, { visible: false })
    );
    ...
    $elems.each(function (index) {
      var $this = $(this),
        value = JSAV.utils.value2type(
          $this.attr("data-value") || $this.html(), // value
  ...
  arrproto.equals = function (otherArray, options) {
    ...
    if ($.isArray(otherArray)) {
      // simple case of array values
      if (!options) {
        // if nothing in options is specified
        len = otherArray.length;
        if (this.size() !== len) {
          // don't compare arrays of different size
          return false;
        }
        for (i = 0; i < len; i++) {
        ...
        // if options
        if ("css" in opts) {
          // if css property given, compare given array to property
          cssprop = opts.css;
          for (i = 0; i < len; i++) {
      ...
      if ("css" in opts) {
        ...
        if ($.isArray(opts.css)) {
          ...
          // if not array, expect it to be a property name string
          cssprop = opts.css;
          for (i = 0; i < len; i++) {
      ...
      if ("class" in opts) {
        ...
        if ($.isArray(opts["class"])) {
          ...
          // if not array, expect it to be a class name string
          clazzname = opts["class"];
          for (i = 0; i < len; i++) {
            equal =
              this.hasClass(i, clazzname) === otherArray.hasClass(i, clazzname);
            if (!equal) {
              return false;
            }
          }
  ...
  arrproto.addClass = function (index, className, options) {
    var indices = JSAV.utils._helpers.normalizeIndices(
      $(this.element).find("li.jsavindex"),
      index,
      ":not(." + className + ")"
    );
  ...
  arrproto.toggleLine = JSAV.anim(function (index, options) {
    ...
    if ($markline.size() === 0 && $mark.size() === 0) {
      // no mark exists yet
      if (opts.markStyle !== null) {
        // mark is not disabled
      ...
      // mark exists already, remove them
      $mark.remove();
      $markline.remove();
    }
    return [index, opts];
  ...
  function barArray(array, options) {
    ...
    $items.each(function (index, item) {
      ...
      var $valueBar = $i.find(".jsavvaluebar"),
        ...
        newBarHeight = Math.round(
          valueBarHeight * (array.value(index) / maxValue)
        );
