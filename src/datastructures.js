...
(function ($) {
  ...
  dsproto.getSvg = function () {
    if (!this.svg) {
      // lazily create the SVG overlay only when needed
  ...
  var Edge = function (jsav, start, end, options) {
    ...
    var visible =
      typeof this.options.display === "boolean" &&
  ...
  edgeproto.layout = function (options) {
    var sElem = this.start().element,
      ...
      fromAngle = normalizeAngle(
        2 * Math.PI - Math.atan2(toY - fromY, toX - fromX)
      ),
      toAngle = normalizeAngle(
        2 * Math.PI - Math.atan2(fromY - toY, fromX - toX)
      ),
      ...
      endStrokeAdjust = this.options["arrow-end"]
        ? strokeWidth * ADJUSTMENT_MAGIC
        : 0,
    ...
    if (
      this.start().value() === "jsavnull" ||
      this.end().value() === "jsavnull"
    ) {
      ...
      if (radius > 0 && angle > topAngle && angle < bottomAngle) {
        ...
        if (angle < bottomAngle && angle > pi) {
          ...
          intersect = lineIntersectCircle(
            { x: dim.x, y: dim.y },
            targetNodeCenter,
            { x: dim.x + dim.width - radius, y: dim.y + dim.height - radius },
            radius
          );
        } else {
          // top right
