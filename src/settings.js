...
(function ($) {
    ...
    return function () {
      ...
      var $elem = $(
        '<div class="jsavrow">' +
          ...
          (rangeSupported ? " " + translate("(slowFast)") : "") +
  ...
  var createInputComponent = function (varname, options) {
    var label,
      ...
      input = $(
        '<input id="jsavsettings-' + varname + '" type="' + opts.type + '"/>'
      );
    if ("label" in opts) {
      label = $(
        '<label for="jsavsettings-' + varname + '">' + opts.label + "</label>"
      );
    }
    if ("value" in opts) {
