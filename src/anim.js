...
(function ($) {
  ...
  AnimatableOperation.prototype.apply = function () {
    ...
    if (typeof retVal === "undefined" || retVal === this.obj) {
      if (
        typeof this.undoeffect === "undefined" ||
        !$.isFunction(this.undoeffect)
      ) {
        this.undoeffect = (function () {
          return function () {
            // we create one that will set the state of obj to its current state
            obj.state(state);
          };
        })();
      }
  ...
  JSAV.init(function () {
    ...
    if ($controls.size() !== 0) {
      ...
      $(
        "<span class='jsavbegin' title='" + tmpTranslation + "'>&lt;&lt;</span>"
      )
        .click(beginHandler)
        .appendTo($controls);
      tmpTranslation = this._translate("backwardButtonTitle");
      $("<span class='jsavbackward' title='" + tmpTranslation + "'>&lt;</span>")
        .click(backwardHandler)
        .appendTo($controls);
      tmpTranslation = this._translate("forwardButtonTitle");
      $("<span class='jsavforward' title='" + tmpTranslation + "'>&gt;</span>")
        .click(forwardHandler)
        .appendTo($controls);
      tmpTranslation = this._translate("endButtonTitle");
      $("<span class='jsavend' title='" + tmpTranslation + "'>&gt;&gt;</span>")
        .click(endHandler)
        .appendTo($controls);
  ...
  function anim(effect, undo) {
    ...
    return function () {
      ...
      if (jsav.options.animationMode === "none" || norecord) {
        // if not recording, apply immediately
  ...
  if (typeof localStorage !== "undefined" && localStorage) {
    // try to fetch a stored setting for speed from localStorage
    var spd = localStorage.getItem("jsav-speed");
    if (spd) {
      // if we have a value, it is a string (from localStorage)
      spd = parseInt(spd, 10);
      if (isNaN(spd)) {
        // if we couldn't parse an int, fallback to default speed
        spd = DEFAULT_SPEED;
      }
    } else {
      // no value in localStorage, go with the default speed
      spd = DEFAULT_SPEED;
    }
    JSAV.ext.SPEED = spd;
  } else {
  ...
  JSAV.ext.step = function (options) {
    var updateRelative =
      options && options.updateRelative === false ? false : true;
    if (updateRelative) {
